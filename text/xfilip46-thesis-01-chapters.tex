...
\label{chapter-data-analyzation}
...
\subsection*{Plotter}
\texttt{plotly}\footnote{https://pypi.org/project/plotly/} is used for plotting the final graphs. It works natively with the dataframe data used in \texttt{pandas}.
...
\section{Basic evaluation strategy}
...
For the evaluationa, a coin to stablecoin transfer has been chosen for the profit evaluation. In the following chapters the metric calculation is discussed. The profit evaluation is detailed when needed.
...
Unfortunately, Benjamin Cowen does not have his risk metric calculation formula publicly available. In a past video TODO: link, he hinted that one piece used are the moving averages. Specifically when you take 50 day MA and you divide it by 50 week MA. That is the first value used.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{figures/experiment-riskmetric.png}
    \caption{Risk Metric Experiment}
    \label{riskmetric-figure}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{figures/experiment-riskmetric2.png}
    \caption{Risk Metric Experiment}
    \label{riskmetric2-figure}
\end{figure}
...
\label{limitations-and-improvements}
...
The API endpoint from CoinGecko for the top 7 trending coins searched by their users could be used to expects pumps of the selected coins.
