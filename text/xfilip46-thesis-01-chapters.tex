...
Correlation with the volume in \$USD traded in one day has been chosen for our strategy. It is a sensible metric. When more people trade the asset the price can be expected to fluctuate a lot---usually up---and vice versa. The data got is from the Binance exchange so slight data imperfection, due to not being the global data, can be expected. Let us first look at how the 24h volume looks when plotted against the price of Bitcoin.

The data correlation between the two series is approximately $0.8$, with $1$ being the best positive correlation. It is not a great correlation, but it is not a bad one either. It gives a basis to work upon. Comparison can be seen in Figures \ref{figure-24volume-ols}, \ref{figure-24volume-lin} and \ref{figure-24volume-log}.
...
The forecasts got are too linera and usually too optimistic to be of any bigger use. Some forecast can be seen in Figure \ref{figure-autots-prediction}
...
Let us put the implemented strategies to use. In this section the correct evaluation strategy is found and each discussed strategy in Section \ref{section-strategies} evaluated.

Some of the strategies, like machine learning optimization, is not evaluated since its usefulness has not been proved in the previous section.
...
Metric based on diminishing returns expected price as described in Section \ref{dim-returns}
