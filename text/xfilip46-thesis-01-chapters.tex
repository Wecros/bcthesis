...
Basically, the thesis can be split into a function and a finite state machine. The function's inputs is the today date and some kind of other data used for calculating the metric (e.g. historical price of Bitcoin). Function then calculates the risk for the current day and passes the value to the finite state machine. The automata then decides on what to do, taking the previous step into the account.

\subsection*{Finite state machine behaviour}
The most basic idea is to transfer the coins to stablecoins.
